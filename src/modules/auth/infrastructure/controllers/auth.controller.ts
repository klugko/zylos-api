...
} from "@modules/auth/application/dto/password-reset.dto";
...
export class AuthController {
  ...
  @ApiOperation({
    summary: "Lister les utilisateurs avec pagination et recherche dynamique",
  })
  ...
  getProfile(@CurrentUser() user: User) {
    return {
      ...
      avatarUrl:
        user.avatarUrl ||
        `https://ui-avatars.com/api/?name=${encodeURIComponent(this.extractInitials(user.fullname))}&size=128&background=random&color=fff&bold=true`,
      phone: user.phone,
      poste: user.poste,
  ...
  @Get("google/callback")
  @UseGuards(AuthGuard("google"))
  @ApiOperation({ summary: "Callback Google OAuth - API JSON" })
  async googleCallbackApi(@Req() req: any) {
  ...
  @Get("google/redirect")
  @UseGuards(AuthGuard("google"))
  @ApiOperation({ summary: "Callback Google OAuth - JSON Response" })
