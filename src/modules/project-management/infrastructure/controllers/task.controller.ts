...
export class TaskController {
  ...
  @Get("enums")
  @ApiOperation({
    summary: "Lister les valeurs autorisées des énumérations Task",
  })
  @ApiResponse({
    status: 200,
  ...
  @Get("project/:projectId/grouped-by-columns")
  @UseGuards(JwtAuthGuard)
  @ApiBearerAuth("JWT-auth")
  @ApiOperation({
    summary: "Lister les tâches d'un projet groupées par colonnes",
  })
  @ApiParam({ name: "projectId", description: "ID du projet" })
  @ApiResponse({ status: 200, description: "Tâches groupées par colonnes" })
  async findByProjectGroupedByColumns(@Param("projectId") projectId: string) {
    return await this.taskRepo.findByProjectGroupedByColumns(projectId);
  }
  ...
  @ApiBearerAuth("JWT-auth")
  @ApiOperation({
    summary: "Mettre à jour une tâche (titre, dates, statut, etc.)",
  })
  @ApiParam({ name: "id", description: "Identifiant unique de la tâche" })
  @ApiBody({
    description:
      "Champs modifiables : titre, description, dates, statut, priorité, assignation, colonne.",
    type: UpdateTaskDto,
  })
      ...
      example: [
        {
          tasks: [
            ...
            },
            {
              ...
              description:
                "Écrire des tests Cypress pour le flux d'inscription.",
            },
          ],
        },
      ],
  ...
  @ApiResponse({
    status: 200,
    description: "Liste des tâches assignées retournée",
  })
  ...
  @UseGuards(JwtAuthGuard)
  @ApiBearerAuth("JWT-auth")
  @ApiOperation({ summary: "Déplacer une tâche vers une autre colonne" })
  @ApiParam({ name: "id", description: "ID de la tâche" })
  @ApiBody({
    schema: {
      type: "object",
      properties: {
        columnId: { type: "string", description: "ID de la nouvelle colonne" },
      },
      required: ["columnId"],
    },
  })
  @ApiResponse({ status: 200, description: "Tâche déplacée avec succès" })
  @ApiResponse({ status: 404, description: "Tâche ou colonne non trouvée" })
  async moveToColumn(
    @Param("id") id: string,
    ...
    const updatedTask = await this.updateTaskUseCase.execute(id, {
